(function(){
  ...
  function applyFemaleProfile(model,profile={}){
    ...
    model.userData.blingShape=morph;
    model.traverse(node=>{if(node.isSkinnedMesh&&node.morphTargetDictionary){const dict=node.morphTargetDictionary,values=node.morphTargetInfluences;values.fill(0);pair(values,dict,'bodyHeavier','bodyThinner',morph.heavy);if(dict.hipsWider!=null)values[dict.hipsWider]=Math.max(0,morph.hips)}if(node.isBone){if(!node.userData.blingBaseScale)node.userData.blingBaseScale=node.scale.clone();node.scale.copy(node.userData.blingBaseScale)}});
    if(!model.userData.blingBaseScale)model.userData.blingBaseScale=model.scale.clone();const base=model.userData.blingBaseScale,heightScale=clamp(height/162,.88,1.14);model.scale.set(base.x,base.y*heightScale,base.z);model.updateMatrixWorld(true);model.userData.refreshBakedSkin?.();
  }
  function installBakedSkinDisplay(model){
    const sources=[];model.traverse(node=>{if(node.isSkinnedMesh&&node.visible)sources.push(node)});const pairs=[];
    sources.forEach(source=>{const geometry=source.geometry.clone(),material=source.material.clone();material.skinning=false;material.morphTargets=false;geometry.morphAttributes={};const display=new THREE.Mesh(geometry,material);display.name=source.name+'Display';display.position.copy(source.position);display.quaternion.copy(source.quaternion);display.scale.copy(source.scale);display.renderOrder=source.renderOrder;source.parent.add(display);source.visible=false;pairs.push({source,display})});
    model.userData.refreshBakedSkin=()=>{model.updateMatrixWorld(true);const shape=model.userData.blingShape||{},bell=(y,c,w)=>Math.exp(-Math.pow((y-c)/w,2));pairs.forEach(({source,display})=>{const input=source.geometry.attributes.position,output=display.geometry.attributes.position,vertex=new THREE.Vector3(),targets=source.geometry.morphAttributes.position||[],influences=source.morphTargetInfluences||[];for(let i=0;i<input.count;i++){vertex.fromBufferAttribute(input,i);for(let j=0;j<targets.length;j++){const amount=influences[j]||0;if(amount){vertex.x+=targets[j].getX(i)*amount;vertex.y+=targets[j].getY(i)*amount;vertex.z+=targets[j].getZ(i)*amount}}if(source.name==='Body'){let width=1+(shape.bust||0)*.09*bell(vertex.y,1.18,.2)+(shape.waist||0)*.11*bell(vertex.y,.94,.16)+(shape.hips||0)*.13*bell(vertex.y,.76,.18)+(shape.shoulders||0)*.08*bell(vertex.y,1.37,.12)+(shape.thighs||0)*.08*bell(vertex.y,.52,.24);if(Math.abs(vertex.x)>.28)width+=((shape.arms||0)*.07)*bell(vertex.y,1.02,.38);vertex.x*=clamp(width,.82,1.22);vertex.z*=clamp(1+(width-1)*.72,.86,1.17)}output.setXYZ(i,vertex.x,vertex.y,vertex.z)}output.needsUpdate=true;display.geometry.computeVertexNormals();display.geometry.computeBoundingSphere()})};
    model.userData.refreshBakedSkin();return pairs;
  }
  async function mountFemaleModel(host,profile){
    ...
    const response=await fetch(new URL('https://threejs.org/examples/models/gltf/Michelle.glb',location.href));if(!response.ok)throw Error('女性基础模型读取失败');const buffer=await response.arrayBuffer();
    return new Promise((resolve,reject)=>new THREE.GLTFLoader().parse(buffer,'',gltf=>{
      host.querySelectorAll('canvas').forEach(x=>x.remove());const scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(24,host.clientWidth/host.clientHeight,.01,100),renderer=new THREE.WebGLRenderer({antialias:true,alpha:true,preserveDrawingBuffer:true});renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.setSize(host.clientWidth,host.clientHeight);renderer.outputEncoding=THREE.sRGBEncoding;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.72;host.appendChild(renderer.domElement);
      ...
      const model=gltf.scene;let skinned=0,bones=0;model.traverse(node=>{if(node.isBone)bones++;if(!node.isMesh)return;if(node.isSkinnedMesh){skinned++;node.normalizeSkinWeights()}node.frustumCulled=false;const count=node.geometry.attributes.position.count,morphAttributes=node.geometry.morphAttributes||{},hasMorph=Object.values(morphAttributes).some(attributes=>attributes?.length);Object.values(morphAttributes).forEach(attributes=>{for(let i=0;i<attributes.length;i++)if(!attributes[i])attributes[i]=new THREE.Float32BufferAttribute(new Float32Array(count*3),3)});node.geometry.computeVertexNormals();if(/Eyes/i.test(node.name))node.material=new THREE.MeshPhysicalMaterial({color:0x30272b,roughness:.32,clearcoat:.4,morphTargets:hasMorph,skinning:true});else if(/Teeth|Tongue/i.test(node.name))node.visible=false;else node.material=new THREE.MeshPhysicalMaterial({color:0xd8a69e,roughness:.72,metalness:0,clearcoat:.03,side:THREE.DoubleSide,skinning:true,morphTargets:hasMorph})});
      if(!skinned||!bones){reject(Error('女性 GLB 缺少有效骨骼'));return}applyFemaleProfile(model,profile);installBakedSkinDisplay(model);let box=new THREE.Box3().setFromObject(model),size=box.getSize(new THREE.Vector3());if(size.z>size.y*2){model.rotation.x=-Math.PI/2;model.updateMatrixWorld(true);box=new THREE.Box3().setFromObject(model);size=box.getSize(new THREE.Vector3())}const center=box.getCenter(new THREE.Vector3());model.position.sub(center);scene.add(model);const fitDistance=(size.y*.56)/Math.tan(THREE.MathUtils.degToRad(camera.fov*.5));camera.position.set(0,size.y*.015,fitDistance);camera.lookAt(0,0,0);const controls=new THREE.OrbitControls(camera,renderer.domElement);controls.enablePan=false;controls.enableDamping=true;controls.dampingFactor=.08;controls.minDistance=fitDistance*.72;controls.maxDistance=fitDistance*1.45;controls.minPolarAngle=Math.PI*.28;controls.maxPolarAngle=Math.PI*.72;controls.target.set(0,0,0);
      let active=true,frame=0;const draw=()=>{if(!active)return;frame=requestAnimationFrame(draw);controls.update();renderer.render(scene,camera)};draw();renderer.domElement.addEventListener('pointerdown',()=>host.classList.add('three-active'),{once:true});const resize=()=>{if(!active)return;camera.aspect=host.clientWidth/host.clientHeight;camera.updateProjectionMatrix();renderer.setSize(host.clientWidth,host.clientHeight)};addEventListener('resize',resize);
      window.__blingFemaleBody={model,scene,camera,renderer,apply:p=>applyFemaleProfile(model,p),skinnedMeshes:skinned,bones,destroy(){active=false;cancelAnimationFrame(frame);removeEventListener('resize',resize);controls.dispose();renderer.dispose()}};resolve(window.__blingFemaleBody);
    },reject));
  }
  function openFemaleBodyEditor(){
    ...
    const host=document.querySelector('#femaleBodyViewport'),state=document.querySelector('#femaleModelState');mountFemaleModel(host,read()).then(()=>state.textContent='我的数字衣模 · 拖动查看 3D').catch(error=>{console.error('Female GLB load failed',error);state.textContent='我的数字衣模'});document.querySelectorAll('[id^="body-"]').forEach(input=>input.addEventListener('input',()=>{updateBmi();window.__blingFemaleBody?.apply(read())}));updateBmi();document.querySelector('#saveBody').onclick=()=>{const profile=read();Object.keys(profile).forEach(key=>profile[key]==null&&delete profile[key]);localStorage.setItem('bling-body',JSON.stringify(profile));state.textContent='身形数据已保存并同步到首页';window.BlingLegacyImport?.toast?.('身体数据与数字衣模已同步 ✦')};
  }
