(function(){
  ...
  function applyFemaleProfile(model,profile={}){
    ...
    model.traverse(node=>{if(!node.isSkinnedMesh||!node.morphTargetDictionary)return;const dict=node.morphTargetDictionary,values=node.morphTargetInfluences;values.fill(0);pair(values,dict,'heightTaller','heightShorter',morph.height);pair(values,dict,'bodyHeavier','bodyThinner',morph.heavy);if(dict.bodyFeminine!=null)values[dict.bodyFeminine]=.72;if(dict.bodySofter!=null)values[dict.bodySofter]=clamp(.25+Math.max(0,morph.heavy)*.55,0,1);pair(values,dict,'bustBigger','bustSmaller',morph.bust);pair(values,dict,'chestWider','chestNarrower',(morph.bust+morph.shoulders)*.38);pair(values,dict,'waistWider','waistNarrower',morph.waist);pair(values,dict,'hipsWider','hipsNarrower',morph.hips);pair(values,dict,'gluteusBigger','gluteusSmaller',(morph.hips+morph.heavy)*.42);pair(values,dict,'shouldersWider','shouldersNarrower',morph.shoulders);pair(values,dict,'thighsThicker','thighsThinner',morph.thighs);pair(values,dict,'armsThicker','armsThinner',morph.arms);pair(values,dict,'legsLonger','legsShorter',morph.legs);pair(values,dict,'neckThicker','neckThinner',morph.neck)});model.updateMatrixWorld(true);
  }
  async function mountFemaleModel(host,profile){
    ...
    const response=await fetch(new URL('assets/models/female-parametric-base.glb?v=2',location.href));if(!response.ok)throw Error('女性基础模型读取失败');const buffer=await response.arrayBuffer();
    return new Promise((resolve,reject)=>new THREE.GLTFLoader().parse(buffer,'',gltf=>{
      ...
      if(!skinned||!bones){reject(Error('女性 GLB 缺少有效骨骼'));return}applyFemaleProfile(model,profile);const box=new THREE.Box3().setFromObject(model),size=box.getSize(new THREE.Vector3()),center=box.getCenter(new THREE.Vector3());model.position.sub(center);scene.add(model);camera.position.set(0,size.y*.015,size.y*1.52);camera.lookAt(0,0,0);const controls=new THREE.OrbitControls(camera,renderer.domElement);controls.enablePan=false;controls.enableDamping=true;controls.dampingFactor=.08;controls.minDistance=size.y*1.22;controls.maxDistance=size.y*2.05;controls.minPolarAngle=Math.PI*.34;controls.maxPolarAngle=Math.PI*.66;controls.target.set(0,0,0);
  ...
  const openBodyWithSliders=()=>{
    openFemaleBodyEditor();
    document.querySelectorAll('[data-body-slider]').forEach(slider=>{
      const input=document.getElementById('body-'+slider.dataset.bodySlider);
      const sync=()=>{const set=input.value!=='';slider.classList.toggle('is-unset',!set);if(set)slider.value=clamp(+input.value,+slider.min,+slider.max);slider.setAttribute('aria-valuetext',set?input.value:'未填写，拖动设置')};
      slider.addEventListener('input',()=>{input.value=slider.value;input.dispatchEvent(new Event('input',{bubbles:true}))});
      input.addEventListener('input',sync);sync();
    });
  };
  window.openBodyEditor3D=openBodyWithSliders;if(window.BlingGeneration)window.BlingGeneration.openBodyEditor=openBodyWithSliders;
})();
